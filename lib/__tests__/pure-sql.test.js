...
test('should parse multiple templates from base template files', function() {
    ...
    expect(templates.hey.query).toEqual('SELECT * FROM I\nWHERE test = "true";');
...
test('should map a template into both a query and its arguments', function() {
    ...
    const baseTemplates = {
        base: '-- name: test\nUPDATE someone SET length = :len, name = :name WHERE id IN (:id*);'
    };
    const templates = pureSql.PG.parse(baseTemplates);

    const mapped = templates.test.mapTemplate(obj);
    expect(mapped).toEqual({query: 'UPDATE someone SET length = $1, name = $2 WHERE id IN ($3,$4);', args: [55, 'testguy', 'testId', 'testId2']});
});
...
test('should leave out parameters that are marked as lists', function() {
    ...
    const templates = pureSql.PG.parse(baseTemplates);
    expect(templates).toMatchObject({
        test: {query: 'SELECT * FROM me\nWHERE a IN (:paramA*) AND b = $1;', params: ['paramB']}
    });
});
...
test('should allow to create templates from dynamic templates', function() {
    ...
    expect(specificTemplate).toMatchObject({
...
test('should also allow to change the parameter format', function() {
    ...
    expect(templates).toMatchObject({
        test: {query: 'SELECT * FROM me\nWHERE a IN (:paramA*) AND b = ?;', params: ['paramB']}
    });
});
...
test('should allow repeating arguments for unnumbered parameters', function() {
    ...
    expect(templates.test.map({'id': 5, 'name': '123'})).toEqual([5, '123', 5]);
});
...
test('should keep the order of dynamic and normal parameters', function() {
    ...
    // TODO: check what this test is testing ^^;
    const templates = pureSql.PG.parse(baseTemplates);
    expect(templates).toMatchObject({
        test: {params: [], query: 'SELECT * FROM me WHERE id IN (:id*), name IN (:name*);'},
        test2: {params: [], query: 'INSERT INTO huff (a,b,c) VALUES :huff** :puff**;'},
        test3: {params: [], query: 'SELECT * FROM :!table1 t1 JOIN :!table2 t2 ON t1.key = t2.key;'},
...
test('should allow using dynamic parameters in multiple places', function() {
    ...
    const templates = pureSql.PG.parse(baseTemplates);
    expect(templates).toMatchObject({
        test: {params: [], query: 'SELECT * FROM :!table t1 JOIN :!table t2 ON t1.key = t2.key WHERE id IN (:ids*) AND oldid IN (:ids*);'},
        test2: {params: [], query: 'INSERT INTO you (a,b,c) VALUES :vals** :vals**;'}
    });
    ...
    expect(mysqlTemplates).toMatchObject({
        test: {params: [], query: 'SELECT * FROM :!table t1 JOIN :!table t2 ON t1.key = t2.key WHERE id IN (:ids*) AND :!table = \'me\' AND oldid IN (:ids*);'},
        test2: {params: [], query: 'INSERT INTO you (a,b,c) VALUES :vals** :vals**;'}
    });
...
test('should allow partial generation with hooks', function() {
    ...
    const templates = pureSql.PG.parse(baseTemplates);
    expect(templates).toMatchObject({
        test: {params: ['b'], query: 'SELECT * FROM table t1 WHERE $1 > 5 AND :*gen :a :b :c*;'}
    });

    let sqlHookGen = function(a, b, c) {
...
test('should allow partial generation with hooks and mixed variables', function() {
    ...
    const templates = pureSql.PG.parse(baseTemplates);
    expect(templates).toMatchObject({
        test: {params: ['b', 'c'], query: 'SELECT * FROM table t1 WHERE $1 > 5 AND :*gen :a :b :c* AND $2 IN (1,5,6);'}
    });

    let sqlHookGen = function(a, b) {
        return {partial: '(a,b) IN (:a,:b)'};
    };
...
test('should allow partial with renaming of hook variables', function() {
    ...
    const templates = pureSql.PG.parse(baseTemplates);
    expect(templates).toMatchObject({
        test: {query: 'SELECT * FROM table t1 WHERE $1 > 5 AND :*gen :a :b :c** AND $1 IN (1,5,6) OR (:c*) = (1,2) OR (:c*) = (4,5);'}
    });
