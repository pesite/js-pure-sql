...
test('should read files from folder by extension', function() {
    ...
    expect(templates).toEqual({Here: {params: [], query: 'SELECT * FROM here;'}, more: {params: ['id'], query: 'SELECT * FROM evenMore WHERE id = $1;'}});
});
...
test('should read Postgresql-compatible files from folder by extension', function() {
    ...
    expect(templates.more).toEqual({params: ['id'], query: 'SELECT * FROM evenMore WHERE id = $1;'});
});
